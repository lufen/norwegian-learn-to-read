...
- **Write Words** — dictation exercise: listen to a word, then tap letter tiles in order to build it (no keyboard needed). Wrong tiles are rejected immediately so the child self-corrects instead of typing and seeing a diff afterward.
...
js/child-confirm.js    Child-safe confirmation overlay (replaces browser confirm())
